...
if __name__ == "__main__":
    ...
    with open(f"./csv/{B}_test_{A}.csv", "r") as f:
        df = f.readlines()

    games_id = [i.split(",", 2)[0] for i in df]
    games = [i.split(",", 2)[-1] for i in df]

    X = np.load(f"./data/{B}/test/{A}.npy")
    ...
    model.load_state_dict(torch.load(f"./weights/{B}/{MODEL}.pth"))
    model.eval()

    with open(f"./{B}_{MODEL}_{A}.csv", "w") as f:
        for i, (data,) in enumerate(tqdm(dataloader)):
            with torch.no_grad():
                y_pred = model(data)
                indices = torch.topk(torch.softmax(y_pred, dim=1), k=5, dim=1)[1]
                y_pred_top5 = indices.cpu().numpy()

            start_idx = i * batch_size
            end_idx = start_idx + y_pred_top5.shape[0]
            for game_id, preds in zip(games_id[start_idx:end_idx], y_pred_top5):
                preds_str = ",".join([decode_label(j) for j in preds])
                f.write(f"{game_id},{preds_str}\n")
